...
const Coordinator = () => {
  ...
  async function verifySignature() {
    let privateKey = process.env.REACT_APP_PRIVATE_COORDINATOR;
    let wallet = new ethers.Wallet(privateKey);
    let messageHash = ethers.utils.id("Coordinator");
    ...
    if (
      wallet.address.toLowerCase() === addressReturn.toString().toLowerCase()
    ) {
      return true;
    } else return false;
  }
  ...
  async function handleApproval(appID) {
   const index=getIndex(appID)
    setLoader(true);
    if ((await verifySignature()) === true) {
      await contractInstance.functions
        .setCoOrdinatorApproval(index, true)
        .then((res) => {
          console.log(res);
          toast.success("Application has been sent to the government", {
  ...
  async function handleCancellation(appID) {
    ...
    if ((await verifySignature()) === true)
      await contractInstance.functions
        .setCoOrdinatorApproval(index, false)
        .then((res) => {
          console.log(res);
          toast
            .success("Application has been Cancelled", {
              position: "bottom-center",
              autoClose: 5000,
              theme: "dark",
            })
                ...
                <div className="flex w-full py-3 px-3 bg-black  rounded-xl shadow">
                  <div className="flex flex-col space-y-4 w-full">
                    ...
                    <div className="flex justify-between gap-x-12">
                      <button
                        type="button"
                        onClick={(e) => handleApproval(application.applicationID)}
